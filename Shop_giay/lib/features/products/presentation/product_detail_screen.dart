...
class ProductDetailScreen extends StatefulWidget {
  ...
  const ProductDetailScreen({super.key, this.product, this.productId})
    : assert(
        ...
        'product or productId is required',
      );
...
class _ProductDetailScreenState extends State<ProductDetailScreen> {
  ...
  final _cartRepo = CartRepository(
    CartApi(Dio(BaseOptions(baseUrl: AppConfig.baseUrl))),
  );
  ...
  void _addToCart(Product p) {
    if (_selectedSize == null) {
      ScaffoldMessenger.of(
        context,
      ).showSnackBar(const SnackBar(content: Text('Vui lòng chọn size trước')));
      return;
    }
...
class _DetailBody extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    final mainUrl = imgs.isNotEmpty
        ? imgs[selectedImage.clamp(0, imgs.length - 1)]
        : '';
          ...
          if (wide) {
            ...
            return SingleChildScrollView(
              child: Padding(
                ...
                child: Column(
                  // Bọc Row vào trong Column để thêm Comment ở dưới
                  ...
                  children: [
                    ...
                    const SizedBox(
                      height: 32,
                    ), // Khoảng cách giữa thông tin SP và bình luận
                  ...
                  child: Opacity(
                    ...
                    child: Container(
                      ...
                      decoration: BoxDecoration(
                        borderRadius: BorderRadius.circular(10),
                        border: Border.all(
                          color: sel ? const Color(0xFFE11D48) : Colors.black12,
                          width: 2,
                        ),
