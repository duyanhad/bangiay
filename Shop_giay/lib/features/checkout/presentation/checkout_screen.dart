...
class _CheckoutScreenState extends State<CheckoutScreen> {
  ...
  Future<void> _placeOrder() async {
    ...
    try {
      ...
      final String methodToSend = (_paymentMethod == 'bank')
          ? 'cod'
          : _paymentMethod;
      ...
      final orderData = {
        ...
        "note": _paymentMethod == 'bank'
            ? "CK ngân hàng: ${_selectedBank?['name']}"
            : "",
      };
      ...
      if (_paymentMethod == 'vnpay') {
        ...
        if (url.isEmpty) throw "Lỗi: Không lấy được link thanh toán VNPay";
  ...
  void _handleSuccess(String message) {
    ...
    // Xóa giỏ hàng
    cartCtrl
        .loadCart(); // Hàm này load lại (thường sẽ reset nếu API trả về rỗng) hoặc dùng hàm clearCart() nếu bạn có
  ...
  void _showWebPaymentDialog() {
    showDialog(
      ...
      builder: (ctx) => AlertDialog(
        ...
        content: const Text(
          "Cửa sổ thanh toán đã mở tab mới.\nSau khi thanh toán xong, hãy bấm xác nhận.",
        ),
        actions: [
          TextButton(
            onPressed: () {
              Navigator.pop(ctx);
              // ✅ Sử dụng context của CheckoutScreen, không phải dialog context
              if (!mounted) return;
            ...
            children: [
              ...
              ListTile(
                ...
                subtitle: Text(
                  ...
                  style: TextStyle(
                    color: _selectedBank != null ? Colors.blue : Colors.grey,
                  ),
                ),
                trailing: IconButton(
  ...
  Widget _buildBankCard(double amount) {
    String qrUrl =
        "https://api.vietqr.io/image/${_selectedBank!['id']}-${_selectedBank!['stk']}-compact2.jpg?amount=${amount.toInt()}&addInfo=ThanhToanDonHang";
          ...
          children: [
            ...
            Container(
              ...
              child: Image.network(
                qrUrl,
                height: 180,
                errorBuilder: (_, __, ___) => const Text("Lỗi tải mã QR"),
              ),
            ),
  ...
  Widget _buildPayButton(double total) => Container(
    ...
    child: ElevatedButton(
      ...
      child: _isProcessing
          ...
          : Text(
              "XÁC NHẬN ĐẶT HÀNG (${total.toStringAsFixed(0)}đ)",
              style: const TextStyle(
                color: Colors.white,
                fontWeight: FontWeight.bold,
                fontSize: 16,
  ...
  Widget _inputField(
    ...
    child: TextFormField(
      ...
      validator: (v) =>
          (v == null || v.trim().isEmpty) ? "Vui lòng nhập $l" : null,
    ),
  );
