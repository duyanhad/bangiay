...
class _ProductsManageScreenState extends State<ProductsManageScreen> {
        ...
        actions: [
          ...
          ElevatedButton(
            ...
            onPressed: () async {
              ...
              final ok = await adminController.deleteProduct(id);
  ...
  void _showProductDialog({Map? product}) {
    ...
    String id = '';
    if (product != null) {
      if (product['_id'] is Map && product['_id']['\$oid'] != null) {
        id = product['_id']['\$oid'];
    ...
    if (product != null && product['variants'] is List) {
      variants = List<Map<String, dynamic>>.from(product['variants']);
    }
                ...
                child: SingleChildScrollView(
                  child: Column(
                    ...
                    children: [
                      ...
                      _modernField("URL hình ảnh", imageCtrl, Icons.image),
                      const SizedBox(height: 20),
                          ...
                          ElevatedButton(
                            ...
                            onPressed: () async {
                              ...
                              final success = isEdit
                                  ? await controller.updateProduct(id, data)
  ...
  Widget _modernField(
    ...
    return TextField(
      ...
      decoration: InputDecoration(
        ...
        focusedBorder: OutlineInputBorder(
          borderRadius: BorderRadius.circular(16),
          borderSide: const BorderSide(color: AdminColors.header1, width: 2),
        ),
      ),
    );
  }
      ...
      child: Wrap(
        ...
        children: variants.map<Widget>((v) {
          ...
          if (qty <= 0) {
            ...
            bg = Colors.orange.withValues(alpha: .15);
  ...
  Widget _productCard(Map p) {
    // FIX $oid
    String id = '';
    if (p['_id'] is Map && p['_id']['\$oid'] != null) {
      id = p['_id']['\$oid'];
    ...
    final name = p['name'] ?? '';
    final price = p['final_price'] ?? p['price'] ?? 0;
          ...
          children: [
            Padding(
              ...
              child: TextField(
                ...
                onChanged: (v) {
                  context.read<AdminController>().searchProducts(v);
                },
              ),
            ),
            Expanded(
              child: controller.isLoading
                  ...
                  : ListView.builder(
                      ...
                      itemBuilder: (_, i) =>
                          _productCard(controller.products[i]),
                    ),
            ),
          ],
        ),
